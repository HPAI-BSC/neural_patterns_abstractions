...
def extract_weights_of_interest(all_interest_nodes, name='patata'):
	...
	np.savez('../data/weights/weights_interest_' + name + '.npz', w=weights,wconv=weights_conv,wfc=weights_fc,wpl=per_layer)
...
def plot_nice_three(interest,origin,destination,name):
	...
	plt.figure()
	plt.hist([interest,origin,destination], histtype='step', density=True, bins=100,label=['interest','origin','destination'])
	plt.legend(loc='upper right')
...
def plot_weights_of_interest(all_interest_nodes, name):
	...
	plot_nice_three(w1, w2, w3, name)
	plot_nice_three(w1_c, w2_c, w3_c, name + '_conv')
	plot_nice_three(w1_fc, w2_fc, w3_fc, name + '_fc')

	for layer in w1pl:
		plot_three(w1pl[layer], w2pl[layer], w3pl[layer], name + layer)
		plot_nice_three(w1pl[layer], w2pl[layer], w3pl[layer], name + layer + '_nice')


	print('all ploted')


def extract_weights_of_interest_with_normal_origin(all_interest_nodes,name='patata'):
	"""
	The main idea of this code is extract the connexion between neurons with income connection such that have value one on
	their corresponding fne value is one.
	:param feture_dict:
...
def extract_weights_of_interest_with_normal_destination(all_interest_nodes,name='patata'):
	"""
	The main idea of this code is extract the connexion between neurons with income connection such that have value one on
	their corresponding fne value is one.
	:param feture_dict:
	...
	np.savez('../data/weights/weights_destiny_' + name + '.npz', w=weights,wconv=weights_conv,wfc=weights_fc,wpl=per_layer)
	return weights, weights_conv, weights_fc, per_layer

from __future__ import division
import numpy as np
import sys
sys.path.append('/gpfs/projects/bsc28/tiramisu_semantic_transfer/tiramisu_source/')
from tiramisu.tensorflow.core.backend import read_embeddings
import os
from collections import Counter, OrderedDict
import matplotlib
matplotlib.use('pdf')
import matplotlib.pyplot as plt

layers = OrderedDict([(u'vgg16_model/relu1_1', (0, 64)), (u'vgg16_model/relu1_2', (64, 128)), (u'vgg16_model/relu2_1', (128, 256)), (u'vgg16_model/relu2_2', (256, 384)), (u'vgg16_model/relu3_1', (384, 640)), (u'vgg16_model/relu3_2', (640, 896)), (u'vgg16_model/relu3_3', (896, 1152)), (u'vgg16_model/relu4_1', (1152, 1664)), (u'vgg16_model/relu4_2', (1664, 2176)), (u'vgg16_model/relu4_3', (2176, 2688)), (u'vgg16_model/relu5_1', (2688, 3200)), (u'vgg16_model/relu5_2', (3200, 3712)), (u'vgg16_model/relu5_3', (3712, 4224)), (u'vgg16_model/relu6', (4224, 8320)), (u'vgg16_model/relu7', (8320, 12416))])

def find_layer_of_feature(feature, embedding):
        """
        This function find the layer correspondent to the given feature in a certain embedding.
        It returns the layer name of the feature.
        :param feature: a feature : int
        :param embedding: a dictionary with keys: ['embeddings', 'image_paths', 'image_labels', 'feature_scheme']
        :return:
        """
        layers = embedding['feature_scheme'][()] ## Ordered dict.
        layer = None
        for i in layers.values():
                if feature in range(i[0], i[1]):
                        layer = i
                        # print(feature, i)
        try:
                layer_name =  list(layers.keys())[list(layers.values()).index(layer)]
                return layer_name
        except:
                print('Feature not in range')
        return None

def extract_synset_name(path):
        """
        /gpfs/projects/bsc28/tiramisu_semantic_transfer/embeddings/1284246
        :param path:
        :return:
        """
        _path = path + '/results.txt'
        synset_name = ''
        with open(_path, 'r') as f:
                for l in f:
                        if 'dataset' in l:
                                synset_name = l.split(' ')[1].strip()
        return str(synset_name)

def read_embedding( job_label,embedding_path,data_path='/gpfs/projects/bsc28/semantic_transfer_scripts/data/', delete=False):
        """
        embedding_path expected:  returns /gpfs/projects/bsc28/tiramisu_semantic_transfer/embeddings/1284246
        :param synset_name:
        :param embedding_path:
        :return:
        """
        synset_name = extract_synset_name(embedding_path)
        print('Going in with', job_label, synset_name)
        embedding_path = embedding_path + '/train/embeddings'
        (results, img_paths, labels, layers_dict) = read_embeddings(path_prefix=embedding_path)
        dmp = np.copy(results == 1)

        # Get subset by synset
        pos_emb = dmp[[x == synset_name for x in labels]]
        #neg_emb = dmp[[x != synset_name for x in labels]]
        if pos_emb.shape[0] == 0:
                print(synset_name, 'is broken')
                return
        print('Size of ' + synset_name + ' embedding', pos_emb.shape)
        #print('Size of complementary embedding', neg_emb.shape)

        # Compute distribution per feature.
        pos_feature_counts = []
        #neg_feature_counts = []
        counter = 0
        for p in pos_emb.T:
        #for p, n in zip(pos_emb.T, neg_emb.T):
                p_unique, p_counts = np.unique(p, return_counts=True)
                #n_unique, n_counts = np.unique(n, return_counts=True)
                # If there is only one value
                if len(p_counts) == 1:
                        if p_unique[0] == True:
                                pos_feature_counts.append(p_counts)
                        else:
                                pos_feature_counts.append(0)
                # if there are two values, get the second which corresponds to True
                else:
                        try:
                                pos_feature_counts.append(p_counts[1])
                        except IndexError:
                                print('An error occurred when processing counts', p_counts)
                # If there is ony one value
                #if len(n_counts) == 1:
                #        # its a full hit!
                #        if n_unique[0] == True:
                #                neg_feature_counts.append(n_counts)
                #        # Full miss
                #        else:
                #                neg_feature_counts.append(0)
                ## if there are two values, get the second which corresponds to True
                #else:
                #        if n_counts[1] > n_counts[0]:
                #                neg_features_maj_ones.append(counter)
                #        try:
                #                neg_feature_counts.append(n_counts[1])
                #        except IndexError:
                #                print('An error occurred when processing counts', n_counts)

                counter += 1
        return pos_feature_counts, pos_emb.shape[0]

def plot_ss_layer_distribution(ss_pos_feature_counts, leaf_name):
    """
    Plot histograms of layer distribution. Stores 2 figures (one normalized, the other one
    with absolute values) in "../plots/" as pdfs.
    :ss_pos_feature_counts: list of triplets. First value counts of length equal to the number of features. Each value indicates the number of images within the synset with a 1 for that feature. Second value is the number of images within the synset. Third value is the name of the synset
    :leaf_name: lemma of the leaf synset
    """
    #which thresholds are going to be considered?
    threshold = 50
    #Initialize the data structures for storing the plot points
    data_points_by_layer = np.zeros((len(ss_pos_feature_counts),len(layers.keys())))
    data_points_by_layer_norm = np.zeros((len(ss_pos_feature_counts),len(layers.keys())))
    #Sort synsets by specificity
    print 'presorting',[x[2] for x in ss_pos_feature_counts]
    ss_pos_feature_counts = sorted(ss_pos_feature_counts, key=lambda tup: tup[1])
    print 'postsorting',[x[2] for x in ss_pos_feature_counts]
    #Generate data per synset
    for idx_s, s in enumerate(ss_pos_feature_counts):
        #Generate data per layer
        for idx_l, l in enumerate(layers.keys()):
            layer_range = layers[l]
            #Get number of features within layer and above threshold
            val = len([x for idx,x in enumerate(s[0]) if idx >= layer_range[0] and idx <= layer_range[1] and x/s[1] > (threshold*0.01)])
            #Store raw data, store normalized data
            data_points_by_layer[idx_s][idx_l] = val
            val = val/(layer_range[1]-layer_range[0])
            data_points_by_layer_norm[idx_s][idx_l] = val
    #plot diagram
    palette = plt.get_cmap('plasma')
    colors = np.linspace(0,1,len(data_points_by_layer))
    types = ['o','*','X','v','s','|','^','>','<','+']
    for idx_s,s in enumerate(data_points_by_layer):
        plt.plot(range(0,len(layers.keys())),s, '-'+types[idx_s], label=ss_pos_feature_counts[idx_s][2], color=palette(colors[idx_s]))
    plt.xticks(np.arange(len(layers.keys())), (layers.keys()), rotation=70, fontsize=6)
    plt.legend(loc='upper right')
    plt.savefig('plots/layer_distribution_pos_feats_semantically_merged_'+leaf_name+'.pdf')
    #plot histogram normalized
    plt.cla()
    #plot diagram
    palette = plt.get_cmap('plasma')
    for idx_s,s in enumerate(data_points_by_layer_norm):
        plt.plot(range(0,len(layers.keys())),s, '-'+types[idx_s], label=ss_pos_feature_counts[idx_s][2], color=palette(colors[idx_s]))
    plt.xticks(np.arange(len(layers.keys())), (layers.keys()), rotation=70, fontsize=6)
    plt.legend(loc='upper right')
    plt.savefig('plots/layer_distribution_pos_feats_semantically_merged_norm_'+leaf_name+'.pdf')


def _recurse_all_hyponyms(synset, all_hyponyms):
    synset_hyponyms = synset.hyponyms()
    if synset_hyponyms:
        all_hyponyms += synset_hyponyms
        for hyponym in synset_hyponyms:
            _recurse_all_hyponyms(hyponym, all_hyponyms)

def _recurse_all_hypernyms(synset, all_hypernyms):
    synset_hypernyms = synset.hypernyms()
    if synset_hypernyms:
        all_hypernyms += synset_hypernyms
        for hypernym in synset_hypernyms:
            _recurse_all_hypernyms(hypernym, all_hypernyms)

"""
This code generates plots showing the distribution of "synset features" through layers. For each synset two plots are generated. Each plot shows the distribution of features per layer, as the threshold of minimum activated images varies. One plot shows the data in absolute number of features, and the other normalizing by the number of features in the layer. Plots are stored in "../plots"
REQUIRES: 
    -Access to the results of step 3 (var step3_data_location).
    -Access to tiramisu methods (see top of the file)
    -The embeddings to be from vgg16 (see top of the file, var layers)

WARNING: This is an extremly inefficient and chaotic code. I'll deny any relation with it. It is also likely to break as any input data it feeds from changes in format.
"""
def main():
        step3_data_location =  '/gpfs/projects/bsc28/tiramisu_semantic_transfer/embeddings/'
        #Get all synset names (this can be done much more efficiently)
        folders =  next(os.walk(step3_data_location))[1]
        working_synset_names = []
        for job_label in folders[2:]:
                _path = step3_data_location + job_label
                try:
                    name = extract_synset_name(_path)
                    #pos_feature_counts, size = read_embedding(job_label,_path)
                    working_synset_names.append(name)
                except IOError:
                    #Likely cause in extract_synset_name (missing results.txt file)
                    print 'WARNING1: SKIPPING CORRUPTED EMBEDDING',_path
                except TypeError:
                    #Likely cause in read_embedding (empty embedding)
                    print 'WARNING2: SKIPPING CORRUPTED EMBEDDING',_path
        print '--Have found ',len(working_synset_names),' synset names.'
        import nltk
        from nltk.corpus import wordnet as wn
        #Load all synsets
        working_synsets = []
        for s in working_synset_names:
            #Find the appropriate entry
            entry_idx = 1
            candidate_s = wn.synset(s+'.n.01')
            while candidate_s.lemmas()[0].name() != s:
                entry_idx+=1
                candidate_s = wn.synset(s+'.n.0'+str(entry_idx))            
            working_synsets.append(candidate_s)
        print '--',len(working_synsets),' synsets have been retrieved.'
        #Find leafs
        leaf_synsets = []
        for s in working_synsets:
            all_hyponyms = []
            _recurse_all_hyponyms(s,all_hyponyms)
            if len(set(working_synsets).intersection(set(all_hyponyms)))==0:
                leaf_synsets.append(s)
        print '--',len(leaf_synsets),' leaf synsets have been found.'
        #For each leaf synset, get the list of hypernyms
        synset_sets = []
        for s in leaf_synsets:
            leaf_name = s.lemmas()[0].name()
            all_hypernyms = []
            _recurse_all_hypernyms(s, all_hypernyms)
            synset_hypernyms = list(set(all_hypernyms).intersection(set(working_synsets)))
            if synset_hypernyms != []:
                synset_hypernyms.append(s)
                synset_sets.append((synset_hypernyms,leaf_name))
        print '--This resulted in ',len(synset_sets),' synset sets to plot.'
        #Get all lemmas of ss synsets
        #This approach is bug-prone, as it may include wrong lemmas/synsets
        #It should be fixed in a future version where the name is the actual synset name
        lemmas = []
        for s_set in synset_sets:
            new_l = []
            new_l.append(s_set[1])
            for s in s_set[0]:
                for l in s.lemmas():
                    new_l.append(l.name())
            lemmas.append(new_l)
        #Compute one plot per synset set
        for idx_ss, ss in enumerate(synset_sets):
            if os.path.isfile('plots/layer_distribution_pos_feats_semantically_merged_'+ss[1]+'.pdf'):
                continue
            ss_pos_features = []
            folders =  next(os.walk(step3_data_location))[1]
            #Walk through all folders
            for job_label in folders[2:]:
                _path = step3_data_location + job_label
                #Check if the synset is within the current synset set
                try:
                    name = extract_synset_name(_path)
                    #if wn.synset(name+'.n.01') not in ss[0]:
                    if name not in lemmas[idx_ss]:
                        continue
                    pos_feature_counts, size = read_embedding(job_label,_path)
                    ss_pos_features.append((pos_feature_counts,size,name))
                except IOError:
                    #Likely cause in extract_synset_name (missing results.txt file)
                    print 'WARNING1: SKIPPING CORRUPTED EMBEDDING',_path
                except TypeError:
                    #Likely cause in read_embedding (empty embedding)
                    print 'WARNING2: SKIPPING CORRUPTED EMBEDDING',_path
            print '--All necessary synsets have been read for the current plot.'
            plot_ss_layer_distribution(ss_pos_features,ss[1])


if __name__ == "__main__":
        from time import time
        ini = time()
        main()
        print(time() - ini)

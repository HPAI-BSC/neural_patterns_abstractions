...
The code identifies all synsets in WordNet (notice this may include synsets not found in the file) which are composed by a given range of images (both min. and max.). Inheritance is assumed, such that images labeled to synset "dog" are counted for synset "mammal", "living_thing", etc.
...
def index_and_hyponims_from_label(ss):
        ...
        try:
                ...
                generate_all_ss(image_synset_file_path)
...
def generate_partition(goal_synset,all_ss, image_synset_file_path, min_synset_freq, max_synset_freq, format='npz'):
        ...
        if len(ss) >= min_synset_freq and len(ss) <= max_synset_freq:
                np.savetxt('../data/all_ss_partitions' + '/synset/' + img_ids_to_text([goal_synset])[0] + '_hypernyms.txt', hyper_list, fmt="%s")
                if format == 'txt':
...
def extract_interest_synsets(image_synset_file_path, min_synset_freq, max_synset_freq):
        ...
        try:
                ...
                interest = interest_synsets(min_synset_freq, max_synset_freq)
...
def main(image_synset_file_path='../data/imagenet2012_val_synset_codes.txt', min_synset_freq = 500, max_synset_freq = 40000):
